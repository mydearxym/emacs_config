...
(defun mydearxym/comment-box (b e)
...
(defun mydearxym/js2-which-function ()
...
(defun mydearxym/run-current-file ()
  ...
  (let* (
    ...
    (if (string-equal ξfSuffix "el") ; special case for emacs lisp
      ...
      (if ξprog-name
          (progn
            (message "Running…")
            (async-shell-command ξcmd-str "*mydearxym/run-current-file output*"))
...
(defun mydearxym/load-yasnippet ()
  (interactive)
...
(defun mydearxym/impatient-mode-hook ()
